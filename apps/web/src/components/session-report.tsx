import type { DiscoverResponse, EvidenceOrigin, VisualIntent } from '@glia/api-client'
 ...
 * transcript and pins in the store, the intent from the distiller, the answer and its citations
 * in the discovery query's cache. So it opens instantly, costs no request and spends no Cala
 * credit, and it cannot fail in a way that takes the session with it.
 *
 ...
 */
function citationsOf(discovery: DiscoverResponse | undefined): EvidenceOrigin[] {
  if (discovery?.status !== 'ok') {
    return []
  }
  const seen = new Map<string, EvidenceOrigin>()
  for (const item of discovery.context) {
    for (const origin of item.origins) {
      const key = origin.document?.url ?? origin.document?.name ?? origin.source?.name
      // A citation with nothing to name is not a citation, so it is not listed.
      if (key && !seen.has(key) && (origin.document?.name || origin.source?.name)) {
        seen.set(key, origin)
      }
    }
  }
...
function asPlainText(
  ...
  discovery: DiscoverResponse | undefined,
  citations: EvidenceOrigin[],
  pinned: PinnedRef[],
  transcript: string,
): string {
  ...
  if (discovery?.answer) {
    blocks.push('', 'RESEARCH', discovery.answer)
  }
  if (citations.length > 0) {
    blocks.push('', 'SOURCES')
    blocks.push(
      ...citations.map((origin) => {
        const name = origin.document?.name ?? origin.source?.name ?? 'Untitled'
        const publisher = origin.source?.name
        const url = origin.document?.url
        return [publisher ? `${name} — ${publisher}` : name, url].filter(Boolean).join(' · ')
      }),
    )
  }
  if (pinned.length > 0) {
...
function ReportBody({ generated, onClose }: ReportBodyProps) {
  ...
  // Same query key as the workpane's: this reads the cache and does not re-fetch.
  ...
  const citations = useMemo(() => citationsOf(discovery), [discovery])
  ...
  async function copy() {
    const text = asPlainText(generated, attributes, discovery, citations, pinned, transcript)
    try {
  ...
  return (
    <div className="report-sheet" role="dialog" aria-modal="true" aria-label="Session report">
      ...
      <article className="report">
        ...
        {discovery?.status === 'ok' && discovery.answer ? (
          <section className="report-section">
            <h2 className="report-heading">Research</h2>
            {/*
              Default element set, no `rehype-raw`, no `dangerouslySetInnerHTML` — the same
              sanitisation the workpane relies on. This is text from hosts we do not control.
            */}
            <div className="report-prose">
              <Markdown>{discovery.answer}</Markdown>
            </div>
          </section>
        ) : null}
              ...
              {citations.map((origin) => {
                ...
                return (
                  <li key={url ?? name}>
                    ...
                    {origin.source?.name ? (
                      <span className="report-publisher">{origin.source.name}</span>
                    ) : null}
                    {/* Printed pages have no links, so the URL is set as text beside the name. */}
                    {url ? <span className="report-url">{url}</span> : null}
                  </li>
                )
              })}
            </ol>
